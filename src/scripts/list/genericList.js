...
import { getData, hasData, updateInto } from "../utils/database.js"
...
genericListTbody.addEventListener("dblclick", e => {
  ...
  editGenericList.classList.remove("hidden")
  delayFocus(editGenericListName)
})

function closeEdit() {
  editGenericListName.value = ""
  editGenericList.classList.add("hidden")
  render()
}
...
editGenericListClear.addEventListener("click", () => {
  let id = Number(editGenericListId.value.trim())
  editGenericListName.value = getData("Generics", `Where id = ${id}`)["name"]
...
editGenericListSave.addEventListener("click", () => {
  let id = Number(editGenericListId.value.trim())
  let name = editGenericListName.value.trim()
  let origianlName = getData("Generics", `where id = ${id}`)["name"]

  if (origianlName === name) {
    closeEdit()
    return
  }

  if (hasData("Generics", "name", name)) {
    showMessege("Invalid generic name", `Name already exists`)
    return
  }

  updateInto("Generics", ["name"], [name], `Where id = ${id}`)
  showMessege("Successfully Updated", `Generic Id: ${id}`)
  closeEdit()
})
