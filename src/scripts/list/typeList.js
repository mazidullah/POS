...
typeListTbody.addEventListener("dblclick", e => {
  ...
  delayFocus(editTypeListName)
})

function closeEdit() {
  editTypeListName.value = ""
  editTypeList.classList.add("hidden")
  render()
}
...
editTypeListClear.addEventListener("click", () => {
  let id = Number(editTypeListId.value.trim())
  editTypeListName.value = getData("Types", `Where id = ${id}`)["name"]
...
editTypeListSave.addEventListener("click", () => {
  let id = Number(editTypeListId.value.trim())
  let name = editTypeListName.value.trim()
  let origianlName = getData("Types", `where id = ${id}`)["name"]
  ...
  if (hasData("Types", "name", name)) {
    showMessege("Invalid type name", `Name already exists`)
    return
  }

  updateInto("Types", ["name"], [name], `Where id = ${id}`)
  showMessege("Successfully Updated", `Type Id: ${id}`)
  closeEdit()
})
